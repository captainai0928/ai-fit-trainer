...
async def coach_advice(
    ...
    언제 호출하나: 사용자가 단순 정보 질의가 아니라 감정·컨디션·고민을 토로할 때 가장 먼저 호출한다.
    감지 대상: 통증 호소("무릎이 아파요"), 정체기("3주째 안 늘어요"), 식단 거부감("식단이 힘들어요"),
    타인 비교 좌절("남들보다 뒤처져요"), 피로/번아웃("너무 지쳤어요"), 입문 막막함("뭐부터 해야 해요?").
    구체적인 대체 운동·회복 루틴·운동 플랜이 필요하면 각각 substitute_exercise, recovery_routine,
    plan_workout을 이어서 호출하라.

    케이스별 코칭 원칙: 부상=안전 최우선(48시간 지속 시 병원 안내) / 정체기=변수 하나만 바꿔 2주 /
    식단 거부감=대체 옵션+80/20 원칙 / 비교 좌절=자기 기준 진전으로 관점 전환 /
    번아웃=휴식도 훈련 / 입문=첫 주는 빈도·시간만 단순하게.
    매칭 없으면 일반 격려와 함께 상태를 구체화해달라는 질문을 반환한다.

    Args:
        message: 사용자 고민/상태 자유 텍스트 (필수)

    Returns:
        str: 감지 케이스 + 코칭 원칙 + 구체 조언(+부상 시 안전 문구) + disclaimer로 구성된 한국어 텍스트.
    """
