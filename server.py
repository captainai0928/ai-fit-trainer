...
async def coach_advice(
    ...
    """사용자의 고민/상태 텍스트에서 6가지 케이스를 감지해 맞춤 코칭 조언을 반환한다.
...
async def substitute_exercise(
    ...
    """통증·불편 부위를 받아 그 부위 부담을 줄인 대체 운동 3종과 주의사항을 반환한다.
...
async def recovery_routine(
    ...
    """운동 후 또는 휴식일에 진행할 강도별 회복 루틴 단계를 반환한다.
...
async def plan_workout(
    ...
    """목표·수준·가용 시간·장비를 받아 오늘 진행할 운동 플랜(워밍업→본운동→쿨다운)을 생성한다.
